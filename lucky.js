  ...
  return `
    <a style="color:white; text-decoration:none; margin-bottom:1rem;" href="slug.html?id=${v.id}">
      <div class="card">
        ...
        <div style="padding-bottom:0.4rem; padding-top:8px; padding-left: 12px; text-align:left;" class="title">${v.title}</div>
        
        <div style="text-align: left; padding-left: 12px;  padding-bottom:0.5rem;" class="desc">${v.description || ""}</div>
        ${v.difficulty ? `<span style="padding-left: 12px;" class="badge">${v.difficulty}</span>` : ""}
      </div>
    </a>
  `;
}
